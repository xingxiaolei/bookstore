...
from books.enums import *   #这里是基本数据
...
class BooksManager(models.Manager):
    ...
    def get_books_by_type(self, type_id, limit=None, sort='default'):
        '''根据type_id查询商品信息'''
        if sort == 'new':
            order_by = ('-create_time',)  #元组
        elif sort == 'hot':
            order_by = ('-sales',)
        elif sort == 'price':
            order_by = ('price',)
        else:
            order_by = ('-pk',) #按照 主键降序排列

        books_li = self.filter(type_id=type_id).order_by(*order_by) #元组解包
...
class Books(BaseModel):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2, verbose_name='商品价格') #max_digits数字最大位数， decimal_places小数最大位数
